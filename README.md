...
> ⚠️ **Important Note:** `ponder=true` does **not** work reliably with `go infinite` mode, especially when paired with `stop` or `quit`. It is recommended to always use `ponder=false` for stable operation.It should not use more than 4096 MB hash,8 threads or 4 multipv
---
...
chmod +x supranova
```
BUT I WILL RECOMMEND USING THE BINARIES FROM OFFICIAL RELEASES AS ABOVE AND EVERYTHING EXCEPT WINDOWS REQUIRE chmod +x , then u dont need the previous methods u can just download them from [here](https://github.com/suprateem-ux/SupraNova-Chess-engine-/releases/tag/v1.0.14)
Here you go, 
```bash
