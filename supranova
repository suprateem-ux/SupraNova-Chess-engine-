...
def see_safe(board: chess.Board, move: chess.Move, use_cache: bool = True) -> int:
    ...
    try:
        ...
        result = gains[0] if gains else 0
...
def quiescence(board: chess.Board, alpha: int, beta: int) -> int:
    """Quiescence search: conservative capture pruning using see_safe()."""
    global node_count
    if timeout():
        raise TimeoutError
    node_count += 1
    ...
    # Delta pruning: stand-pat too low
    if stand + FUTILITY_MARGIN < alpha and not board.is_check():
        return alpha

    # Generate tactical moves: captures, promotions (to queen), and checks.
    moves: List[chess.Move] = []
    for m in board.legal_moves:
        if board.is_capture(m):
            # Prefer to evaluate capture safety with simulator SEE; prune only
            # clearly losing captures.
            try:
                s = see_safe(board, m)
            except Exception:
                # fallback to faster SEE if simulation fails
                try:
                    s = see(board, m)
                except Exception:
                    s = 0
            # Only skip captures that are clearly losing by margin.
            if s < -50:
                continue
            moves.append(m)
